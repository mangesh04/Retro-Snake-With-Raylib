...
void applyTheme(Theme t)
{
    ...
    {
        COL_BLUE = {15, 177, 219, 255};
        COL_GREEN = {135, 241, 97, 255};
        COL_DIM = {80, 80, 80, 255};
        COL_BG = {8, 8, 8, 225};
    }
}
...
void updateDesktopBuddy()
{
    ...
    if (expanding_h)
    {
        ...
        resizeFromCenter(newW, TV_LINE_H);
        if (current_win_w >= (float)windowWidth)
        {
            expanding_h = false;
            expanding_v = true;
        }
...
void drawDesktopBuddy()
{
    ...
    if (buddy_standing || buddy_attacking)
    {
        ...
        if (buddy_focused && currentTheme == Theme::RETRO)
        {
