...
static void initPostProcess()
{
    ...
    if (compBorderColorLoc == -1 || compBorderThickLoc == -1)
...
void initializeWindow()
{
    ...
    retroFont = LoadFontEx("mini_pixel-7.ttf", 64, 0, 0);
...
void update()
{
    ...
    if (type == AppType::INTERNAL_SNAKE)
        updateGame();
    if (type == AppType::INTERNAL_POMODORO)
        updatePomodoro();
    if (type == AppType::INTERNAL_PONG)
        updatePong();
